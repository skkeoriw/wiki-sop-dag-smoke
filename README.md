...
```text
使用 youtube-wiki，帮我把这个 YouTube 链接跑进 skkeoriw/wiki-sop-dag-smoke: https://www.youtube.com/watch?v=dQw4w9WgXcQ
```
...
bash <(curl -fsSL https://skill.vyibc.com/youtube-wiki.sh) \
  --endpoint=https://<your-youtube-wiki-endpoint> \
  --mode=trigger \
...
```bash
youtube-wiki trigger --repo skkeoriw/wiki-sop-dag-smoke --url 'https://www.youtube.com/watch?v=5MgBikgcWnY'
youtube-wiki trigger --repo skkeoriw/wiki-sop-dag-smoke --url 'https://www.youtube.com/watch?v=5MgBikgcWnY' --watch
youtube-wiki status --repo skkeoriw/wiki-sop-dag-smoke --pipeline-id '<pipeline_id>'
